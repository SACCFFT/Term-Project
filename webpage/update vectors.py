...
for anime in animeList:
	...
	print tags
	tags = constants.preprocessing(tags)
	print tags
	totalFreq += [tags]

	vector = constants.stringToTag(anime.normalizedVector)
	dbtags += [vector]
	score = anime.score
	if score > 0:
		avg += score
...
print
tagFreq = constants.tagFreq(totalFreq)
print tagFreq
tagFrequency = constants.tagFreq(tagFreq)
print tagFrequency
u.member.tagTotal = constants.tagToString(tagFrequency)
IDFvector = constants.IDFvector(tagFrequency, len(animeList))
print IDFvector
u.member.IDFvector = constants.tagToString(IDFvector)
